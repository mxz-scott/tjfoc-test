...
//    @After
    ...
    public void shareTransfer_StoryEgTest()throws Exception{
        ...
        gdEquityCode = "gdEC" + Random(12);
        List<Map> shareList = gdConstructShareListV1(gdAccount1,6000,0);
        ...
        respShareList = gdConstructQueryShareList(gdAccount1,4000,0,2000,"",respShareList);
        respShareList = gdConstructQueryShareList(gdAccount1,500,1,300,"",respShareList);
        respShareList = gdConstructQueryShareList(gdAccount5,1000,0,0,"",respShareList);
        respShareList = gdConstructQueryShareList(gdAccount5,500,1,0,"",respShareList);
        ...
        assertEquals(true,query.contains("{\"equityCode\":\"" + gdEquityCode +
                "\",\"shareProperty\":1,\"sharePropertyCN\":\"" + mapShareENCN().get("0") + "\",\"totalAmount\":500,\"lockAmount\":300}"));
        ...
        assertEquals(true,query.contains("{\"equityCode\":\"" + gdEquityCode +
                "\",\"shareProperty\":1,\"sharePropertyCN\":\"" + mapShareENCN().get("0") + "\",\"totalAmount\":500,\"lockAmount\":0}"));
    ...
    public void shareTransfer_lockMatchEqcode()throws Exception{
        ...
        uf.shareTransfer(gdAccountKeyID4,gdAccount4,1000,gdAccount6,1,EqCode1,0,"20200908001",true);
        uf.shareTransfer(gdAccountKeyID5,gdAccount5,1000,gdAccount6,0,EqCode3,0,"20200908001",true);
    ...
    public void transfer_MatchCase()throws Exception{
        ...
        String response = uf.shareTransfer(gdAccountKeyID1,gdAccount1,1000,gdAccount5,0,
                gdEquityCode.toLowerCase(),0,"test202008280952",false);
        assertEquals("500",JSONObject.fromObject(response).getString("state"));
        assertEquals("查询余额出错",JSONObject.fromObject(response).getString("message"));

        response = uf.shareTransfer(gdAccountKeyID1,gdAccount1,1000,gdAccount5,0,
                gdEquityCode.toUpperCase(),0,"test202008280952",false);
        assertEquals("500",JSONObject.fromObject(response).getString("state"));
        assertEquals("查询余额出错",JSONObject.fromObject(response).getString("message"));
    }
}
