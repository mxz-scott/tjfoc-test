...
public class IpcrProcessTest {
    ...
    public static void init() throws Exception {
        ...
        IpcrCommonFunc copyrightCommonFunc = new IpcrCommonFunc();
...
//        assertEquals("400", JSONObject.fromObject(response).getString("state"));
        ...
        //账户注册接口，keyId为空
        response = ipcr.crAcountRegister("", "user");
        assertEquals("400", JSONObject.fromObject(response).getString("state"));
...
//        assertEquals("400", JSONObject.fromObject(response).getString("state"));
        ...
        response = ipcr.crAccountQuery(USERADDRESS1, "123");
        assertEquals("500", JSONObject.fromObject(response).getString("state"));
        assertEquals(true, response.contains(" Smart contract does not exist"));

        //（小程序）订单预支付接口,商户内部订单号outTradeNo为空
        response = ipcr.crPayPrepartion("", "数字藏品", OPENID, 1, "", "",
        ...
        response = ipcr.crPayPrepartion(String.valueOf(constructUnixTime(0)), "", OPENID, 1, "", "",
                false, "", "", constructUnixTime(100), ATTACH);
        assertEquals("400", JSONObject.fromObject(response).getString("state"));
        assertEquals(true, response.contains("'Desc' failed on the 'required' tag"));

        //（小程序）订单预支付接口,小程序的唯一标识openId为空
        response = ipcr.crPayPrepartion
        ...
        response = ipcr.crPayPrepartionH5("", "数字藏品", "Wap",
                1, "", "", false, "10.1.1.1", "", constructUnixTime(100), ATTACH);
        assertEquals("400", JSONObject.fromObject(response).getString("state"));
        assertEquals(true, response.contains("'OutTradeNo' failed on the 'required' tag"));

        //（H5）订单预支付接口,商品描述desc为空
        response = ipcr.crPayPrepartionH5((String.valueOf(constructUnixTime(0))), "", "Wap",
                1, "", "", false, "10.1.1.1", "", constructUnixTime(100), ATTACH);
        assertEquals("400", JSONObject.fromObject(response).getString("state"));
        assertEquals(true, response.contains("'Desc' failed on the 'required' tag"));

        //（H5）订单预支付接口,场景类型type为空
        response = ipcr.crPayPrepartionH5((String.valueOf(constructUnixTime(0))), "数字藏品", "",
                1, "", "", false, "10.1.1.1", "", constructUnixTime(100), ATTACH);
        assertEquals("400", JSONObject.fromObject(response).getString("state"));
        assertEquals(true, response.contains("'Type' failed on the 'required' tag"));

        //（H5）订单预支付接口,订单总金额total为空
        response = ipcr.crPayPrepartionH5((String.valueOf(constructUnixTime(0))), "数字藏品", "Wap",
...
//        assertEquals("400", JSONObject.fromObject(response).getString("state"));
    ...
    public void crProcessTest() throws Exception {
        ...
        ipcrCommonFunc.verifyAccountQuery(USERADDRESS1, SCADDRESS1, artworkId, SYMBOL, false);

        //艺术品审核信息存储
        String response = ipcr.crOrderStore(constructData("SHBH", 8), artworkId, "上技所", "yyds",
        ...
        ipcrCommonFunc.verifyArtworkHistory(artworkId, USERADDRESS1, storeHash);

        //艺术品发行上链
        response = ipcr.crArtworkIssue(USERKEYID1, SCADDRESS1, artworkId, ARTHASH);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);
        ...
        //查询用户1账户艺术品,返回正确数据
        ipcrCommonFunc.verifyAccountQuery(USERADDRESS1, SCADDRESS1, artworkId, SYMBOL, true);
        ...
        //取消订单接口（小程序）
        response = ipcr.crOrderClose(outTradeNo, "取消支付");
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        storeHash = JSONObject.fromObject(response).getJSONObject("data").getString("txId");
        commonFunc.verifyTxDetailField(storeHash, "store", "0", "0", "0");
        ...
        response = ipcr.crPayPrepartionH5(outTradeNo, "数字藏品", "WAP", 1, "", "", false,
                "10.1.1.1", "", constructUnixTime(0), ATTACH);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));

        //取消订单接口（H5）
        response = ipcr.crOrderCloseH5(outTradeNo, "取消支付");
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        storeHash = JSONObject.fromObject(response).getJSONObject("data").getString("txId");
        commonFunc.verifyTxDetailField(storeHash, "store", "0", "0", "0");

        //生成艺术品证书接
        response = ipcr.crArtworkCertPicture(String.valueOf(constructUnixTime(0)), "证书", "艺术品", "123",
    ...
    public void crIssueMultiTest() throws Exception {
        ...
        //艺术品系列初始化
        String response = ipcr.crArtworkScinit(symbol1, MAX, BROKERADDRESS1, TYPENO, BASEURL);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);
        ...
        response = ipcr.crArtworkScinit(symbol2, MAX, BROKERADDRESS1, TYPENO, BASEURL);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);
        ...
        //艺术品状态更新
        response = ipcr.crArtworkScupdate(scAddress1, artworkId11, constructUnixTime(0), true);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));

        String artworkId12 = String.valueOf(constructUnixTime(0));
        log.info("@@@@@@artworkId12:" + artworkId12);
        response = ipcr.crArtworkScupdate(scAddress1, artworkId12, constructUnixTime(0), true);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        sleepAndSaveInfo(2000);

        String artworkId21 = String.valueOf(constructUnixTime(0));
        log.info("@@@@@@artworkId21:" + artworkId21);
        response = ipcr.crArtworkScupdate(scAddress2, artworkId21, constructUnixTime(0), true);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        sleepAndSaveInfo(2000);

        String artworkId22 = String.valueOf(constructUnixTime(0));
        log.info("@@@@@@artworkId22:" + artworkId22);
        response = ipcr.crArtworkScupdate(scAddress2, artworkId22, constructUnixTime(0), true);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        sleepAndSaveInfo(2000);

        //艺术品发行上链
        response = ipcr.crArtworkIssue(USERKEYID1, scAddress1, artworkId11, ARTHASH);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);
        String txId111 = JSONObject.fromObject(response).getJSONObject("data").getString("txId");
        response = ipcr.crArtworkIssue(USERKEYID1, scAddress1, artworkId12, ARTHASH);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);
        String txId121 = JSONObject.fromObject(response).getJSONObject("data").getString("txId");
        response = ipcr.crArtworkIssue(USERKEYID1, scAddress2, artworkId21, ARTHASH);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);
        String txId211 = JSONObject.fromObject(response).getJSONObject("data").getString("txId");
        response = ipcr.crArtworkIssue(USERKEYID1, scAddress2, artworkId22, ARTHASH);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);
        ...
        response = ipcr.crArtworkIssue(USERKEYID2, scAddress1, artworkId11, ARTHASH);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);
        String txId112 = JSONObject.fromObject(response).getJSONObject("data").getString("txId");
        response = ipcr.crArtworkIssue(USERKEYID2, scAddress1, artworkId12, ARTHASH);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);
        String txId122 = JSONObject.fromObject(response).getJSONObject("data").getString("txId");
        response = ipcr.crArtworkIssue(USERKEYID2, scAddress2, artworkId21, ARTHASH);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);
        String txId212 = JSONObject.fromObject(response).getJSONObject("data").getString("txId");
        response = ipcr.crArtworkIssue(USERKEYID2, scAddress2, artworkId22, ARTHASH);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);
        ...
        //查询用户1账户艺术品,返回正确数据
        ipcrCommonFunc.verifyAccountQuery(USERADDRESS1, scAddress1, artworkId11, symbol1, true);
        ipcrCommonFunc.verifyAccountQuery(USERADDRESS1, scAddress1, artworkId12, symbol1, true);
        ipcrCommonFunc.verifyAccountQuery(USERADDRESS1, scAddress2, artworkId21, symbol2, true);
        ipcrCommonFunc.verifyAccountQuery(USERADDRESS1, scAddress2, artworkId22, symbol2, true);

        //艺术品流转历史
        ipcrCommonFunc.verifyArtworkHistory(artworkId11, USERADDRESS1, txId111);
        ipcrCommonFunc.verifyArtworkHistory(artworkId12, USERADDRESS1, txId121);
        ipcrCommonFunc.verifyArtworkHistory(artworkId21, USERADDRESS1, txId211);
        ipcrCommonFunc.verifyArtworkHistory(artworkId22, USERADDRESS1, txId221);

        ipcrCommonFunc.verifyArtworkHistory(artworkId11, USERADDRESS1, txId112);
        ipcrCommonFunc.verifyArtworkHistory(artworkId12, USERADDRESS1, txId122);
        ipcrCommonFunc.verifyArtworkHistory(artworkId21, USERADDRESS1, txId212);
        ipcrCommonFunc.verifyArtworkHistory(artworkId22, USERADDRESS1, txId222);
    }
    ...
    public void crArtworkIssueInvalidTest() throws Exception {
        ...
        //艺术品状态更新
        response = ipcr.crArtworkScupdate(scAddress, artworkId1, constructUnixTime(0), true);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        response = ipcr.crArtworkScupdate(scAddress, artworkId2, constructUnixTime(0), true);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        response = ipcr.crArtworkScupdate(scAddress, artworkId3, constructUnixTime(0), true);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        response = ipcr.crArtworkScupdate(scAddress, artworkId4, constructUnixTime(0), true);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));

        //艺术品发行上链，发行用户1，用户2成功，发行用户3失败
        for (int i = 0; i < max / 2; i++) {
            response = ipcr.crArtworkIssue(USERKEYID1, scAddress, artworkId1, ARTHASH);
            assertEquals("200", JSONObject.fromObject(response).getString("state"));
            commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                    utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);

            response = ipcr.crArtworkIssue(USERKEYID2, scAddress, artworkId1, ARTHASH);
            assertEquals("200", JSONObject.fromObject(response).getString("state"));
            commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                    utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);
        }
        response = ipcr.crArtworkIssue(USERKEYID3, scAddress, artworkId1, ARTHASH);
        assertEquals("500", JSONObject.fromObject(response).getString("state"));

        //艺术品发行上链，发行艺术品种类1/2/3成功，4失败
        response = ipcr.crArtworkIssue(USERKEYID1, scAddress, artworkId2, ARTHASH);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);
        response = ipcr.crArtworkIssue(USERKEYID2, scAddress, artworkId3, ARTHASH);
        assertEquals("200", JSONObject.fromObject(response).getString("state"));
        commonFunc.sdkCheckTxOrSleep(commonFunc.getTxHash(globalResponse, utilsClass.sdkGetTxHashType20),
                utilsClass.sdkGetTxDetailTypeV2, SLEEPTIME);
        response = ipcr.crArtworkIssue(USERKEYID2, scAddress, artworkId4, ARTHASH);
        assertEquals("500", JSONObject.fromObject(response).getString("state"));
    ...
    public void crDuplicateAccountRegisterTest() throws Exception {
        ...
        String response = ipcr.crAcountRegister(BROKERKEYID1, "broker");
        assertEquals("500", JSONObject.fromObject(response).getString("state"));
