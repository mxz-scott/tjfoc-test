...
public class TestLicence {
    ...
    public void resetPeerEnv()throws Exception{
        ...
        mgtool.queryPeerListNo(peer1IPPort,basePeerNo);
    }
    ...
    public void testLicGenAndDec() throws Exception{
        ...
        mgtool.checkParam(PEER1IP,"./license create -m 12:11 -p 10.1.3.240 -d 100 -n 6 -v 2.0","invalid MAC address");
        assertEquals(deLicence(PEER1IP,"peer.lic").contains("open peer.lic: no such file or directory"),true);

        mgtool.checkParam(PEER1IP,"./license create -m 02:42:fc:a2:5b:1b -p 10.1 -d 100 -n 6 -v 2.0","invalid IP address");
        assertEquals(deLicence(PEER1IP,"peer.lic").contains("open peer.lic: no such file or directory"),true);

        mgtool.checkParam(PEER1IP,"./license create -m 02:42:fc:a2:5b:1b -p 10.1.3.240 -d 0.5 -n 6 -v 2.0","invalid argument");
        assertEquals(deLicence(PEER1IP,"peer.lic").contains("open peer.lic: no such file or directory"),true);

        mgtool.checkParam(PEER1IP,"./license create -m 02:42:fc:a2:5b:1b -p 10.1.3.240 -d 5 -n 0.5 -v 2.0","invalid argument");
        assertEquals(deLicence(PEER1IP,"peer.lic").contains("open peer.lic: no such file or directory"),true);

        mgtool.checkParam(PEER1IP,"./license create -m 02:42:fc:a2:5b:1b -p 10.1.3.240 -d 5 -n 0 -v 2.0","success");

        rsp = deLicence(PEER1IP,"peer.lic");
        assertEquals(rsp.contains("PeerNum:0"),true);

        //解析证书使用无效参数
        mgtool.checkParam(PEER1IP,"./license decode -p ./crypt/key.pem","data Illegal");
    ...
    public void testLicValidForPeer()throws Exception{
        ...
        mgtool.startPeer(PEER2IP);
        Thread.sleep(STARTSLEEPTIME);
        mgtool.checkParam(PEER2IP,"./toolkit health -p " + PEER2RPCPort,"connection error");
...
//        Thread.sleep(STARTSLEEPTIME);
        ...
        //动态加入节点247
        mgtool.addPeer("join",peer1IPPort,ipType+PEER3IP,tcpType+tcpPort,rpcPort,"update failed");
        mgtool.startPeer(PEER3IP);
        Thread.sleep(STARTSLEEPTIME);
        mgtool.queryPeerListNo(peer1IPPort,basePeerNo); //检查节点247已经启动成功
        ...
        mgtool.startPeer(PEER3IP);
        Thread.sleep(STARTSLEEPTIME);
        mgtool.checkParam(PEER1IP,"./toolkit health -p "+PEER3IP+":"+rpcPort,"connection error");
