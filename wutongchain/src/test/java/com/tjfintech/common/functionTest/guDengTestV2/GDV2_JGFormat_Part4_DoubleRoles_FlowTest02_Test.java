 ...
 * 多角色场景用例补充
 * 主体既可以挂牌登记 又可以开户
 * 本测试类测试 挂牌登记 发行股份后进行开户
 */

public class GDV2_JGFormat_Part4_DoubleRoles_FlowTest02_Test {
...
//        uf.checkJGHeaderOpVer(blockHeight,endHeight);
    ...
    @Test
    public void TC05_EnterpriseRegNoCreateAcc() throws Exception {
        //确认初始主体版本为-1 即无主体对象标识存在
        assertEquals(issueSubVer,Integer.parseInt(gdCF.getObjectLatestVer(gdCompanyID)));

        uf.regTestUnit("1",false,gdCompanyID);

        //确认主体版本为0 即挂牌登记 0
        assertEquals(0,Integer.parseInt(gdCF.getObjectLatestVer(gdCompanyID)));
    }
...
//        assertEquals("200",JSONObject.fromObject(query).getString("state"));
    ...
    @Test
    public void TC071_EnterpriseSubCreateAcc() throws Exception {
        //确认初始主体版本为0
        assertEquals(0,Integer.parseInt(gdCF.getObjectLatestVer(gdCompanyID)));

        Map enSubInfo = gdBF.init01EnterpriseSubjectInfo();
        accountInfo = uf.createAccForEnterpriseSub(gdCompanyID,enSubInfo);
