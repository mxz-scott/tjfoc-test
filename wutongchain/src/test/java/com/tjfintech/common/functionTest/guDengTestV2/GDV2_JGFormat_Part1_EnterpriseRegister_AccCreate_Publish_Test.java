...
//        gdBefore.gdCreateAccout();
    ...
    //企业 股权类 登记 变更为机构会员
    @Test
    public void TCN015_enterpriseRegisterEquityToMemberCheckFormat() throws Exception {
        regTestUnit("1", false);
        regTestUnit("4", false);
    }
    ...
    //企业 股权类 登记 变更为机构会员
    @Test
    public void TCN016_MemberToEnterpriseRegisterEquityCheckFormat() throws Exception {
        regTestUnit("4", false);
        regTestUnit("1", false);
    }
    ...
    //重复机构会员登记
    @Test
    public void TCN017_MemberCheckFormat() throws Exception {
        regTestUnit("4", false);
        regTestUnit("4", false);
    }
    ...
    //企业 股权类 登记
    @Test
    public void TestObjectIDLength64() throws Exception {
        gdEquityCode = Random(64).toLowerCase();
        gdCompanyID = "P1Re" + Random(60).toLowerCase();
        regTestUnit("1", false);

        gdEquityCode = gdEquityCode.toUpperCase();
        gdCompanyID = gdCompanyID.toUpperCase();
        regTestUnit("1", false);
    }

    //    @Test
    public void TestObjectIDLength128() throws Exception {
        gdEquityCode = Random(128).toLowerCase();
        gdCompanyID = "P1Re" + Random(124).toLowerCase();
        regTestUnit("1", false);
    ...
    public void TCN014_createAccTestCheckFormat() throws Exception {
        ...
        String cltNo = "tester00" + Random(12);
        ...
        JSONArray jsonArray = JSONArray.parseArray(uriInfo.get("storeData").toString());
        assertEquals(chkSubURI,jsonArray.getJSONObject(0).getString("uri"));
        assertEquals(chkSHAccURI,jsonArray.getJSONObject(1).getString("uri"));
        assertEquals(chkFundAccURI,jsonArray.getJSONObject(2).getString("uri"));
...
//        String storeData2 = minio.getFileFromMinIO(minIOEP,jgBucket,"fund_a8dee7aa95e94e13baac526782dca87/0","");
    ...
    public void regTestUnit(String type, Boolean bSetEmpty) throws Exception {
        ...
        switch (type) {
            case "1":
                prodInfo = gdBF.init03EquityProductInfo();
            ...
            case "4":
                ...
                break;
            default:
                assertEquals("非法类型" + type, false, true);
        }
...
//        assertEquals("200", net.sf.json.JSONObject.fromObject(response).getString("state"));
        ...
        if (!type.equals("4")) {
            ...
            //校验监管报送的顺序（"subject","account","product","transactionreport","registration","settlement","infodisclosure）
            assertEquals(chkProdURI, jsonArray.getJSONObject(1).getString("uri"));
        }
        assertEquals(true, gdCF.bContainJGFlag(uriInfo.get("storeData").toString()));//确认meta信息包含监管关键字
