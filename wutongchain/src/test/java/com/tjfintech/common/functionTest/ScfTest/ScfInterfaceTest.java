...
public class ScfInterfaceTest {
    ...
    public void Test001_IssuingApply() throws Exception {
        ...
        String response2 = scf.IssuingApply("3228bf4f6c3d20b641fb508cfddcf578c1fbd92b64b962327b476bc9fb123456", companyID1, coreCompanyKeyID, PIN, tokenType, levelLimit, expireDate, supplyAddress1, amount);
        assertThat(response2, containsString("400"));
        assertThat(response2, containsString("error"));
        assertThat(response2, containsString("get  enterprise err:rpc error: code = Unknown desc = Smart contract does not exist"));

        log.info("资金方commpanyid为空");
        String response3 = scf.IssuingApply(AccountAddress, "", coreCompanyKeyID, PIN, tokenType, levelLimit, expireDate, supplyAddress1, amount);
...
//        assertThat(response6, containsString("Key: 'KeyID' Error:Field validation for 'KeyID' failed on the 'len' tag"));
        log.info("pin码为空");
        String response7 = scf.IssuingApply(AccountAddress, companyID1, coreCompanyKeyID, "", tokenType, levelLimit, expireDate, supplyAddress1, amount);
        ...
        log.info("不存在的pin码");
        String response8 = scf.IssuingApply(AccountAddress, companyID1, coreCompanyKeyID, "789", tokenType, levelLimit, expireDate, supplyAddress1, amount);
        ...
        String response9 = scf.IssuingApply(AccountAddress, companyID1, coreCompanyKeyID, PIN, "", levelLimit, expireDate, supplyAddress1, amount);
        ...
        log.info("受让地址为空");
        String response10 = scf.IssuingApply(AccountAddress, companyID1, coreCompanyKeyID, PIN, tokenType, levelLimit, expireDate, "", amount);
        ...
        log.info("开立额度为空");
        String response11 = scf.IssuingApply(AccountAddress, companyID1, coreCompanyKeyID, PIN, tokenType, levelLimit, expireDate, supplyAddress1, "");
        ...
        log.info("开立额度为负数");
        String response12 = scf.IssuingApply(AccountAddress, companyID1, coreCompanyKeyID, PIN, tokenType, levelLimit, expireDate, supplyAddress1, "-1");
        assertThat(response12, containsString("400"));
        assertThat(response12, containsString("error"));
        assertThat(response12, containsString( "Token amount must be a valid number and less than"));

        log.info("资产有效期为空");
        String response13 = scf.IssuingApply(AccountAddress, companyID1, coreCompanyKeyID, PIN, tokenType, levelLimit, null, supplyAddress1,amount);
        ...
        log.info("开立额度不足");
        String response14= scf.IssuingApply(AccountAddress, companyID1, coreCompanyKeyID, PIN, tokenType, levelLimit, expireDate, supplyAddress1, "10000000");
        ...
        log.info("数值超过小数点后6位");
        String response15= scf.IssuingApply(AccountAddress, companyID1, coreCompanyKeyID, PIN, tokenType, levelLimit, expireDate, supplyAddress1, "100.123456789");
...
//        assertThat(response21, containsString("has been spent"));
    ...
    public void Test0023_PayingConfirm() throws Exception {
        ...
        response9 = scf.PayingConfirm(PlatformAddress, QFJGAddress, companyID1, list2, platformKeyID, platformPIN, tokenType, comments);
        assertThat(response9, containsString("400"));
        assertThat(response9, containsString("error"));
        assertThat(response9, containsString("销毁申请失败： Insufficient Balance!"));
    ...
    public void Test0027_PayingConfirm() throws Exception {
        ...
        response9 = scf.PayingConfirm(PlatformAddress, QFJGAddress, companyID1, list3, platformKeyID, platformPIN, tokenType, comments);
        assertThat(response9, containsString("400"));
        assertThat(response9, containsString("error"));
    ...
    public void Test0029_Finacingback() throws Exception {
        ...
        log.info("txid为空");
        String response10 = scf.FinacingBack(PlatformAddress, platformKeyID, PIN, supplyID2, "", comments);
        ...
        response10 = scf.FinacingBack("", platformKeyID, PIN, supplyID2, "dPvql1mJ/ubwpTsPiP6fUzSjSIg6KmasC8htnpSIlrQ=", comments);
        assertEquals("400", JSONObject.fromObject(response10).getString("state"));
        assertEquals(true, response10.contains("'PlatFormAddress' Error:Field validation for 'PlatFormAddress' failed on the 'required' tag"));

        log.info("PlatformkeyID为空");
        response10 = scf.FinacingBack(PlatformAddress, "", PIN, supplyID2, "aaaaa", comments);
        assertEquals("400", JSONObject.fromObject(response10).getString("state"));
        assertEquals(true, response10.contains("'PlartformKeyID' Error:Field validation for 'PlartformKeyID' failed on the 'len' tag"));

        log.info("PIN为空");
        response10 = scf.FinacingBack(PlatformAddress,platformKeyID, "", supplyID2, "dPvql1mJ/ubwpTsPiP6fUzSjSIg6KmasC8htnpSIlrQ=", comments);
        assertEquals("400", JSONObject.fromObject(response10).getString("state"));
        assertEquals(true, response10.contains("'PIN' Error:Field validation for 'PIN' failed on the 'required' tag"));

        log.info("keyid为空");
        response10 = scf.FinacingBack(PlatformAddress, platformKeyID, PIN, "", "dPvql1mJ/ubwpTsPiP6fUzSjSIg6KmasC8htnpSIlrQ=", comments);
        assertEquals("400", JSONObject.fromObject(response10).getString("state"));
        assertEquals(true, response10.contains("'KeyID' Error:Field validation for 'KeyID' failed on the 'len' tag"));

        log.info("Comments为空");
        response10 = scf.FinacingBack(PlatformAddress, platformKeyID, PIN, supplyID2, "dPvql1mJ/ubwpTsPiP6fUzSjSIg6KmasC8htnpSIlrQ=", "");
        assertEquals("400", JSONObject.fromObject(response10).getString("state"));
    ...
    public void Test0030_AccountInform() throws Exception {
        ...
        String response10 = scf.AccountInform("","BwAAAGZpbmFuY2UDAAAAYWJjAgQAAABzenRq8AAAAExTMHRMUzFDUlVkSlRpQlFWVUp NU1VNZ1MwVlpMUzB0TFMwS1");
        assertEquals("400", JSONObject.fromObject(response10).getString("state"));
        assertEquals(true, response10.contains("'ContractAddress' Error:Field validation for 'ContractAddress' failed on the 'required' tag"));

        log.info("comments为空");
        response10 = scf.AccountInform(PlatformAddress,"");
        assertEquals("400", JSONObject.fromObject(response10).getString("state"));
        assertEquals(true, response10.contains("'Comments' Error:Field validation for 'Comments' failed on the 'required' tag"));
