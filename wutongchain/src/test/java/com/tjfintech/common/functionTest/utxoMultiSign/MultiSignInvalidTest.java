...
public class MultiSignInvalidTest {
    ...
    //多签地址
    private static String multiaddr1="Spya6Nxu3gJfNXpFYn8DQUDUrCMfvqktNmARrsK1GvGj7Jwq1q2";
    private static String multiaddr2="Sok87KmHU1UxpuWPxsEiS9FEBkznSywkP688FszVL91euQUKyG1";
    /**
     * 创建多签地址
     */
    @Test
    ...
    public void TC1335_issuetoken(){
        String issuetoken;
        issuetoken = multiSign.issueTokenCarryPri("11","test211","10000000",UtilsClass.PRIKEY6,"111","zz");//	MultiAddr字段传入无效字段
        assertThat(issuetoken, containsString("Invalid multiple address(from addr)"));
        issuetoken = multiSign.issueTokenCarryPri("4QqVU8DvcZNWQ7mAiuq8SFzZkhKW27PRAgo91Q716KqvK3jYxo","test211","10000000",UtilsClass.PRIKEY1,"zz");
        assertThat(issuetoken, containsString("Invalid multiple address(from addr)"));//	MultiAddr字段传入错误的地址
        issuetoken = multiSign.issueTokenCarryPri(multiaddr2,"","10000000",UtilsClass.PRIKEY6,"111","zz");
        assertThat(issuetoken, containsString("TokenType shouldn't be empty"));//Tokentype字段为空
        issuetoken = multiSign.issueTokenCarryPri(multiaddr2,"test211","",UtilsClass.PRIKEY6,"111","zz");
        assertThat(issuetoken, containsString("Amount must be greater than 0 and less than 900000000"));//	Amount字段为空值
        issuetoken = multiSign.issueTokenCarryPri(multiaddr2,"test211","-1",UtilsClass.PRIKEY6,"111","zz");
        assertThat(issuetoken, containsString("Amount must be greater than 0 and less than 900000000"));//	Amount字段为负值
        issuetoken = multiSign.issueTokenCarryPri(multiaddr2,"test211","90000000000",UtilsClass.PRIKEY6,"111","zz");
    ...
    public void TC1358_transfer(){
        ...
        transfer = multiSign.Transfer("abc", "111","", multiaddr2, tokenList);
        assertThat(transfer, containsString("Private Key must be base64 string"));//	Prikey传入非base64编码字符的值
        transfer = multiSign.Transfer("YWJjeHg=","111", "", multiaddr2, tokenList);
        ...
        transfer = multiSign.Transfer(UtilsClass.PRIKEY6, "111","", "", tokenList);
    ...
    public void addissueaddressRemovePri(){
        String addissueaddress = multiSign.addissueaddressRemovePri(multiaddr2,multiaddr1);
