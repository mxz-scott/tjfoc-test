...
//        uf.calJGDataEachHeight();
    ...
    public void TCN031_ObjectId_sameSubDiffProd() throws Exception {
        ...
        assertEquals(true,JSONObject.parseObject(response).getString("message").contains("在OSS中已经存在"));
    ...
    public void TCN032_ObjectId_diffSubSameProd() throws Exception {
        ...
        response= gd.GDEnterpriseResister(gdContractAddress,gdEquityCode,1000,enSubInfo,
                prodInfo,null,null);
        assertEquals("400",JSONObject.parseObject(response).getString("state"));
        assertEquals(true,JSONObject.parseObject(response).getString("message").contains("对应的产品在系统中已经存在"));
    }
